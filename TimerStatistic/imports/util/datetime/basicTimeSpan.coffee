...
class BasicTimeSpan
  ...
  getTimeSpanObject: (time, span = 1, spanlist = ['年','月','日','时','分']) =>
    ...
    if not time then t = new Date() else t = new Date(time)
  ...
  getYearObject : (t) =>
    ...
    yearObject =
      type: 'year'
      year: time.year()
      start: Moment(time).startOf 'year'
      end: Moment(time).endOf 'year'
  ...
  getMonthObject : (t) =>
    ...
    monthObject =
      ...
      month: time.month() + 1
      year: time.year()
      start: Moment(time).startOf 'month'
      end: Moment(time).endOf 'month'
  ...
  getHourObject : (t) =>
    ...
    hourObject =
      ...
      month: time.month() + 1
      year: time.year()
      start: Moment(time).startOf 'hour'
      end: Moment(time).endOf 'hour'
  ...
  numberSpans: (total, span, value, t, type) =>
    ...
    _.map [0...spanCount], (i) =>
      ...
      if value < end and value >= start
        if type == 'minute'
          result =
            start: Moment(t)[type](start).second(0).millisecond(0)
            end: Moment(t)[type](end - 1).second(59).millisecond(999)
            position: i
        else if type == 'second'
          result =
            start: Moment(t)[type](start).millisecond(0)
            end: Moment(t)[type](end - 1).millisecond(999)
            position: i
    result
  ...
  getMinuteObject : (t, span) =>
    ...
    if span > 60
      ...
      r =
        start: Moment(t).startOf 'minute'
        end: Moment(t).endOf 'minute'
        position: 0
